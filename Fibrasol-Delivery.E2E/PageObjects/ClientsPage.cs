...
public class ClientsPage
{
    ...
    // Selectors
    private const string NewClientButton = "button[data-bs-target='#clientModal']";
    private const string DataTable = "#clientsTable";
    private const string Modal = "#clientModal";
    private const string ModalTitle = "#clientModalLabel";
    private const string NameInput = "#clientName";
    private const string SaveButton = "#clientModal .btn-primary-custom";
    private const string CancelButton = "#clientModal .btn-secondary";
    private const string EditButton = ".btn-edit";
    private const string DeleteButton = ".btn-delete";
    private const string SearchInput = "#clientsTable_filter input";
    private const string TableRows = "#clientsTable tbody tr";

    public ClientsPage(IPage page, string baseUrl)
    {
        _page = page;
        _baseUrl = baseUrl;
    }
    ...
    {
        await _page.GotoAsync($"{_baseUrl}/clientes");
    }
    ...
    public async Task ClickNewClientAsync()
    {
        await _page.ClickAsync(NewClientButton);
    ...
    public async Task SearchAsync(string searchText)
    {
        ...
        await _page.WaitForTimeoutAsync(500); // Wait for DataTable filter
    }
    ...
    {
        return await _page.TextContentAsync(ModalTitle);
    }

    public async Task<string?> GetNameInputValueAsync()
    {
        return await _page.InputValueAsync(NameInput);
    }

    public async Task CreateClientAsync(string name)
    {
        await ClickNewClientAsync();
    ...
    public async Task<bool> HasClientWithNameAsync(string name)
    {
